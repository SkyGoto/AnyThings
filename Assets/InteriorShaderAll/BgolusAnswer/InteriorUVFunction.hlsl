...
float2 ConvertOriginalRawUVToInteriorUV(float2 originalRawUV, float3 viewDirTangentSpace, float roomMaxDepth01Define)
{
	...
	float3 hitPosBoxSpace = viewRayStartPosBoxSpace + shortestHitRayLength * viewRayDirBoxSpace; // 通过射线判断，正真应该取的uv值，以及景深
	//============================================================================
	...
	// assumes camera with an fov of 53.13 degrees (atan(0.5))  // 如果开口不为(atan(0.5)) 则在射线判断时uv范围会超过 [-1,-1,-1] ~ [1,1,1]之间
	//hard to explain, visual result = transform nonlinear depth back to linear
	float realZ = saturate(interp) / depthScale + 1;
	interp = 1.0 - (1.0 / realZ);  //   - (1.0 / realZ) 是的范围限制为[-1~0)， interp 范围为[0~1) 即景深化为线性
	interp *= depthScale + 1.0;

	//linear iterpolate from wall back to near
	float2 interiorUV = hitPosBoxSpace.xy * lerp(1.0, 1 - roomMaxDepth01Define, interp);  // 更贴近线性当 interp -> 1 时几乎为直线
