Shader "Custom/InteriorMapping - simple"
{
        ...
        SubShader
        {
                ...
                Pass
                {
                        ...
                        {
                                float tangentSign = tangentOS.w * unity_WorldTransformParams.w;  // 叉积遵循右手定则， 图像中坐标为左手定则，取unity_WorldTransformParams换向
                                float3 bitangentOS = cross(normalOS, tangentOS.xyz) * tangentSign;  // 副切线方向
                                return float3(dot(inputDirOS, tangentOS.xyz),dot(inputDirOS, bitangentOS),dot(inputDirOS, normalOS));  // 坐标系转换 x y z
                        }
                        ...
                        v2f vert(appdata v)
                        {
                                ...
                                float3 camPosObjectSpace = mul(unity_WorldToObject, float4(_WorldSpaceCameraPos, 1.0)).xyz;  // 摄像机再物体坐标系下的坐标
                                float3 viewDirObjectSpace = v.vertex.xyz - camPosObjectSpace; // 摄像机到物体的方向
